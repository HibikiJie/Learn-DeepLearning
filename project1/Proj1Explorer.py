...
class Explorer:
    ...
    def __init__(self):
        '''设置网络参数'''
        self.p_confidence_threshold = 0.6  # 建议值0.6
        self.p_nms_threshold = 0.6 # 建议值0.5
        self.r_confidence_threshold = 0.5  # 建议值0.6
        self.r_nms_threshold = 0.5  # 建议值0.5
        self.o_confidence_threshold = 0.99999  # 建议值0.97
...
if __name__ == '__main__':
    ...
    explorer = Explorer()
    video_capture = cv2.VideoCapture("http://admin:admin@192.168.42.129:8081/video")
    ...
    while True:
        ...
        cors = boxes[:, 0:4]
        img = cv2.rectangle(img,(boxes[:,0:2]),(boxes[:,2:4]),color=(0,0,255),thickness=2)
        cv2.imshow('Jk',img)
